...
describe("GET /api/health", () => {
  ...
  describe("authenticated response (Bearer CRON_SECRET)", () => {
    ...
    it("marks authenticated health degraded when Stripe checkout is configured without the webhook secret", async () => {
      const originalSecret = process.env.CRON_SECRET;
      process.env.CRON_SECRET = "test-secret";
  ...
  describe("database failure handling", () => {
    ...
    it("coerces explicit external cron mode back to internal on Railway", async () => {
      const originalSecret = process.env.CRON_SECRET;
      const originalMode = process.env.CRON_RUNNER_MODE;
      const originalRailwayEnv = process.env.RAILWAY_ENVIRONMENT;
      process.env.CRON_SECRET = "test-secret";
    ...
    it("does not stay degraded when a job has a newer successful rerun", async () => {
      const originalSecret = process.env.CRON_SECRET;
      const originalMode = process.env.CRON_RUNNER_MODE;
      process.env.CRON_SECRET = "test-secret";
      process.env.CRON_RUNNER_MODE = "external";
