...
export function ShareComparison({ modelNames, slugs }: ShareComparisonProps) {
  const [copied, setCopied] = useState(false);

  const url =
    typeof window !== "undefined"
      ? `${window.location.origin}/compare?models=${slugs.join(",")}`
      : `/compare?models=${slugs.join(",")}`;
  ...
  const handleTwitterShare = () => {
    const names = modelNames.slice(0, 3).join(" vs ");
