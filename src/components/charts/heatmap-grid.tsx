...
import type { TooltipState } from "@/hooks/use-heatmap-tooltip";
...
export function HeatmapGrid({
  ...
  const activeBenchmarks = benchmarks.filter((b) =>
    models.some((model) => model.scores[b.slug] !== null && model.scores[b.slug] !== undefined)
  );

  // Sort models
  const sortedModels = [...models].sort((a, b) => {
    if (!sortBenchmark) return a.rank - b.rank;
  ...
  // Scale column width based on number of benchmarks
          ...
          >
            ...
            {activeBenchmarks.map((benchmark) => {
              ...
              return (
                <div
                  key={benchmark.slug}
                  onClick={() => onColumnSort(benchmark.slug)}
            ...
            {sortedModels.map((model, rowIdx) => (
              <>
                ...
                >
                  ...
                  <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.25)", marginLeft: "auto", flexShrink: 0 }}>
                    {model.provider}
                  </span>
                </div>
                ...
                {activeBenchmarks.map((benchmark) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={(e) => onCellHover(e, model.name, benchmark.name, score)}
