...
describe("benchmark score trust helpers", () => {
  it("filters to trusted structured benchmark sources only", () => {
    const rows = [
      ...
      { id: "bench-2", source: "provider-blog" },
      { id: "bench-3", source: "arena-hard-auto" },
      { id: "bench-4", source: null },
    ];
    ...
    expect(countTrustedStructuredBenchmarkScores(rows)).toBe(2);
  });
  ...
  it("collects unique model ids from trusted structured benchmark rows", () => {
    const rows = [
      ...
      { model_id: "model-1", source: "livebench" },
      { model_id: "model-2", source: "provider-blog" },
      { model_id: "model-3", source: "swe-bench" },
    ];
