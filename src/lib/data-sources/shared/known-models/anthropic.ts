...
export const ANTHROPIC_KNOWN_MODELS: Record<string, KnownModelMeta> = {
  ...
  "claude-fable-5": {
    name: "Claude Fable 5",
    description:
      "Anthropic's most capable widely released model for demanding reasoning and long-horizon agentic work. It is proprietary, generally available through the Claude API and supported cloud platforms, and cannot run locally as an open-weight model. Safety classifiers can refuse or route some requests to a fallback model, and Fable use requires 30-day data retention.",
    category: "multimodal",
    context_window: 1000000,
  ...
  "claude-haiku-4-5": {
    ...
    description:
      "Anthropic's fastest current Claude model, designed for high-volume and latency-sensitive workloads with near-frontier intelligence at its lowest current API price point.",
    category: "multimodal",
    ...
    license: "commercial",
    license_name: null,
    website_url: "https://www.anthropic.com/news/claude-haiku-4-5",
    modalities: ["text", "image"],
    capabilities: {
      vision: true,
      tool_use: true,
...
export function canonicalizeAnthropicModelId(modelId: string): string {
  ...
  const familyAliasMatches: Array<[RegExp, string]> = [
    ...
    [
      /^claude-haiku-4-5(?:-\d{8})?(?:-batch)?(?:-v\d+)?$/,
      "claude-haiku-4-5",
    ],
